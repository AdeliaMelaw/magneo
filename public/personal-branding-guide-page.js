(function () {
  ...
  function bindGuideAnchors() {
    ...
    document.querySelectorAll('a[href="#guide-form"]').forEach(function (link) {
      ...
      link.addEventListener('pointerdown', function () { keyboardActivation = false; });
      link.addEventListener('click', function (event) {
        event.preventDefault();
        formSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        if (keyboardActivation || event.detail === 0) {
          window.setTimeout(function () { firstField.focus({ preventScroll: true }); }, 450);
        }
