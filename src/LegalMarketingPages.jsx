...
function LegalHero({ page, hub = false }) {
  return <section className={`lm-hero ${hub ? 'lm-hero-hub' : ''}`}><div className="container lm-hero-grid"><div className="lm-hero-copy"><nav className="lm-breadcrumb" aria-label="Breadcrumb"><Link to="/">Home</Link><span>→</span>{hub ? <span>Law Firm Marketing</span> : <><Link to="/law-firm-marketing/">Law Firm Marketing</Link><span>→</span><span>{(page.h1 === 'Marketing for family-law firms' ? 'Family Law Firms' : page.h1.replace('Marketing for ', ''))}</span></>}</nav><h1>{page.h1}</h1><p>{page.description}</p><div className="lm-actions"><Link className="btn" to="/contact/#contact-enquiry">{page.primary}</Link><a className="lm-button-secondary" href={page.secondaryHref || '#website-examples'}>{page.secondary}</a></div></div>{hub && <Link className="lm-hero-preview" to={websiteConcepts.immigration.path}><img src={websiteConcepts.immigration.image} alt={websiteConcepts.immigration.alt}/><span>Website concept</span><strong>{websiteConcepts.immigration.title}</strong><small>View demo →</small></Link>}</div></section>;
}
...
function PracticeLinks({ current }) { const links=Object.entries(practicePages).map(([slug,page])=>[(page.h1 === 'Marketing for family-law firms' ? 'Family Law Firms' : page.h1.replace('Marketing for ','')),`/law-firm-marketing/${slug}/`]).filter(([,path])=>path!==`/law-firm-marketing/${current}/`); return <section className="lm-practice-links" aria-label="Explore other practice areas"><div className="container"><strong>Explore other practice areas</strong><div>{links.map(([label,path])=><Link key={path} to={path}>{label}</Link>)}</div></div></section>; }
