...
function ServicePage({ name }){ const title = serviceTitleOverrides[name] || titleFromSlug(name); const kind = kindFromText(title); const ai = name?.startsWith('ai-') || name?.includes('ai-') || name?.includes('compliance-aware-ai'); const intro = ai ? `${title} for regulated industries, built with human review, compliance guardrails, and measurable conversion paths.` : `A compliance-aware ${title.toLowerCase()} service page for regulated industries, built around authority, qualified demand, and measurable conversion.`; const serviceProcess = serviceProcesses[name]; useSeo(`${title} | Magneo`, intro, `${BASE}/services/${name}/`); return <><Hero label={`Service · ${title}`} title={`${title}<br/>for <em>regulated markets.</em>`} intro={intro} stat={ai ? 'AI' : 'SEO'} statText="Current URL preserved for SEO and launch migration."/><section className="section"><div className="container"><div className="label">Who this is for</div><h2>Built for teams where trust is the conversion asset.</h2><div className="grid four">{Object.values(industries).map((i)=><Link className="card" to={i.route} key={i.route}><small>{i.short}</small><h3>{title}</h3><p>{i.intro}</p></Link>)}</div></div></section><Process heading={serviceProcess?.heading} items={serviceProcess?.items}/><RelatedLinks/><CTA/></>; }
