...
import PersonalInjuryFamilyTest from './PersonalInjuryFamilyTest.jsx';
...
const coreServices = [
  ...
  ['AI Automation for Regulated Industries', '/services/ai-automation-for-regulated-industries-magneo/', 'Intelligent workflows for regulated teams, follow-up, intake, reporting, and conversion.'],
...
const serviceMenus = [
  ['Website Design', '/services/website-design-for-regulated-professional-industries-magneo/', [['Website Design for Tech Companies & SaaS Products', '/services/website-design-for-tech-companies-saas-products-magneo/'], ['Website Design for Healthcare Clinics & Doctors', '/services/website-design-for-healthcare-clinics-doctors-magneo/'], ['Website Design for Financial Advisors & Wealth Firms', '/services/website-design-for-financial-advisors-wealth-firms-magneo/'], ['Website Design & Rebrand for Law Firms', '/services/website-design-rebrand-for-law-firms-magneo/']]],
  ...
  ['Social Media & LinkedIn Marketing', '/services/social-media-linkedin-marketing-for-regulated-industries/', [['LinkedIn Growth for Law Firms & Legal', '/services/linkedin-growth-law-firms/'], ['LinkedIn Growth for Financial Advisors & FinTech', '/services/linkedin-growth-financial-advisors/'], ['Social Media for Medical Clinics & Healthcare Providers', '/services/social-media-linkedin-leadership-for-healthcare-providers-magneo/'], ['Social Media for Tech & SaaS Companies', '/services/social-media-linkedin-leadership-for-tech-saas-ai-companies-magneo/'], ['Crypto and AI Social Media', '/services/crypto-and-ai-social-media/']]],
  ['AI Marketing Automation', '/services/ai-automation-for-regulated-industries-magneo/', [['AI Marketing Automation for Law Firms & Legal Departments', '/services/ai-automation-for-law-firms-legal-departments-magneo/'], ['AI Marketing Automation for Financial Advisors', '/services/ai-automation-for-financial-advisors-firms-fintech-magneo/'], ['AI Marketing Automation for Tech, SaaS & AI Companies', '/services/ai-marketing-automation-for-tech-saas-ai-companies-magneo/'], ['AI Automation for Healthcare Providers & Clinics', '/services/ai-automation-for-healthcare-providers-clinics-magneo/']]],
  ['AI-Powered Digital Marketing', '/services/ai-powered-digital-marketing/', [['AI SEO', '/services/ai-seo/'], ['AI Social Media Marketing', '/services/ai-social-media-marketing/'], ['AI UGC & AI Video Production', '/services/ai-ugc-ai-video-production/'], ['AI Web Design & Conversion', '/services/ai-web-design-conversion/'], ['AI Content Marketing', '/services/ai-content-marketing/'], ['Compliance-Aware AI Workflows', '/services/compliance-aware-ai-workflows/']]],
  ['PPC & Landing Pages', '/services/ppc-landing-pages-for-regulated-industries/', [['PPC & Landing Pages for Law Firms', '/services/ppc-landing-pages-for-law-firms-magneo/'], ['PPC & Landing Pages for Healthcare & MedTech', '/services/ppc-landing-pages-for-healthcare-medtech/'], ['PPC & Landing Pages for Financial Advisors & FinTech', '/services/ppc-landing-pages-for-financial-advisors-fintech-magneo/']]],
...
const aiServices = [
  ...
  ['Governance', 'Compliance-Aware AI Workflows', '/services/compliance-aware-ai-workflows/', 'Human review, claim checks, disclaimers, approval steps, and brand voice controls before anything goes live.']
];
...
const serviceTitleOverrides = {
  ...
  'social-media-linkedin-leadership-for-healthcare-providers-magneo': 'LinkedIn Marketing for Healthcare Providers',
...
function HomeMenu(){ return <div className="nav-item nav-home"><Link className="nav-trigger" to="/">Home</Link><div className="nav-home-menu"><Link to="/about/">About Magneo</Link></div></div>; }
const legalConceptRoutes = {'/portfolio/legal-websites/personal-injury-classic':PersonalInjuryClassic,'/portfolio/legal-websites/personal-injury-bold':PersonalInjuryBoldTest,'/portfolio/legal-websites/litigation-editorial':LitigationEditorialTest,'/portfolio/legal-websites/brain-injury-3d':BrainInjury3DTest,'/portfolio/legal-websites/personal-injury-cinematic':PersonalInjuryCinematicTest,'/portfolio/legal-websites/personal-injury-family-focused':TestLawyer6,'/portfolio/legal-websites/personal-injury-family-focused/test':PersonalInjuryFamilyTest,'/portfolio/legal-websites/immigration-welcome':ImmigrationWelcome,'/portfolio/notary-services':NotaryDocumentDesk};
function Layout({ children }) { const { pathname } = useLocation(); const cleanPath=pathname.replace(/\/$/,'')||'/'; const Concept=legalConceptRoutes[cleanPath]; if(Concept) return <Concept/>; if(cleanPath==='/portfolio/legal-websites') return <Navigate to="/portfolio/" replace/>; if(cleanPath==='/test6') return <TestLawyer6/>; if(['/test','/test2','/test3','/test4','/test5'].includes(cleanPath)) return children; if(cleanPath==='/portfolio') children=<PortfolioTest/>; const contactPage=cleanPath==='/contact'; const usesPortfolioFooter=cleanPath==='/portfolio'; const usesAboutFooter=cleanPath==='/about'; const footerDescription=usesPortfolioFooter||usesAboutFooter?'Digital marketing for regulated industries. Based in Toronto, serving Canada and the USA.':undefined; return <><nav className="nav"><div className="container nav-inner"><Link className="brand" to="/">Magneo</Link><div className="nav-links"><HomeMenu/><Mega label="Services" to="/services/" groups={serviceMenus}/><Mega label="Industries" to="/industries/" groups={Object.values(industries).map((i)=>[i.label,i.route,i.subs])}/><Link to="/portfolio/">Portfolio</Link><a href={BLOG}>Insights & Blog</a>{contactPage?<a href="#contact-enquiry">Contact</a>:<Link to="/contact/">Contact</Link>}</div>{contactPage?<a className="btn" href="#contact-enquiry">Contact Adele</a>:<Link className="btn" to="/contact/">Book a call</Link>}</div></nav><main>{children}</main><Footer description={footerDescription} showLocation={!usesPortfolioFooter&&!usesAboutFooter}/></>; }
...
function Process(){ return <section className="section dark"><div className="container"><div className="label">How we work</div><h2>Strategy first, then execution with guardrails.</h2><div className="process">{process.map(([n,t,d])=><div className="process-row" key={n}><b>{n}</b><div><h3>{t}</h3><p>{d}</p></div></div>)}</div></div></section>; }
...
function IndustriesHub(){ useSeo('Industries We Serve | Magneo','Marketing systems for law, finance, healthcare, and technology companies.',`${BASE}/industries/`); return <><Hero label="Industries we serve" title="Marketing built for your industry,<br/>not <em>adapted from someone else.</em>" intro="Every industry we serve has its own compliance rules, buyer psychology, and trust signals." stat="22+"/><section className="section"><div className="container"><div className="grid">{Object.values(industries).map((i)=><Link className="card" to={i.route} key={i.route}><small>{i.short}</small><h3>{i.label}</h3><p>{i.intro}</p></Link>)}</div></div></section><RelatedLinks/><CTA/></>; }
...
function ServicePage({ name }){ const title = serviceTitleOverrides[name] || titleFromSlug(name); const kind = kindFromText(title); const ai = name?.startsWith('ai-') || name?.includes('ai-') || name?.includes('compliance-aware-ai'); const intro = ai ? `${title} for regulated industries, built with human review, compliance guardrails, and measurable conversion paths.` : `A compliance-aware ${title.toLowerCase()} service page for regulated industries, built around authority, qualified demand, and measurable conversion.`; useSeo(`${title} | Magneo`, intro, `${BASE}/services/${name}/`); return <><Hero label={`Service · ${title}`} title={`${title}<br/>for <em>regulated markets.</em>`} intro={intro} stat={ai ? 'AI' : 'SEO'} statText="Current URL preserved for SEO and launch migration."/><section className="section"><div className="container"><div className="label">Who this is for</div><h2>Built for teams where trust is the conversion asset.</h2><div className="grid four">{Object.values(industries).map((i)=><Link className="card" to={i.route} key={i.route}><small>{i.short}</small><h3>{title}</h3><p>{i.intro}</p></Link>)}</div></div></section><Process/><RelatedLinks/><CTA/></>; }
