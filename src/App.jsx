...
function Layout({ children }) { const { pathname } = useLocation(); const cleanPath=pathname.replace(/\/$/,'')||'/'; const Concept=legalConceptRoutes[cleanPath]; if(Concept) return <Concept/>; if(cleanPath==='/portfolio/legal-websites') return <Navigate to="/portfolio/" replace/>; if(cleanPath==='/portfolio') children=<PortfolioTest/>; const contactPage=cleanPath==='/contact'; return <><nav className="nav"><div className="container nav-inner"><Link className="brand" to="/">Magneo</Link><div className="nav-links"><HomeMenu/><Mega label="Services" to="/services/" groups={serviceMenus}/><Mega label="Industries" to="/industries/" groups={Object.values(industries).map((i)=>[i.label,i.route,i.subs])}/><Link to="/portfolio/">Portfolio</Link>{contactPage?<a href="#contact-enquiry">Contact</a>:<Link to="/contact/">Contact</Link>}<SiteSearch items={searchItems} mobile/></div><SiteSearch items={searchItems}/>{contactPage?<a className="btn" href="#contact-enquiry">Contact Adele</a>:<Link className="btn" to="/contact/">Book a call</Link>}</div></nav><main>{children}</main><Footer/></>; }
